...
import { ArrowLeft, Github, ArrowUpRight, Calendar, BookOpen } from "lucide-react";
...
export async function generateMetadata(
    ...
    const project = await getTutorial(slug);
...
export default async function TutorialDetailsPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const project: IProject | null = await getTutorial(slug);
            ...
            <div className="bg-muted/30 border-b pt-20 pb-10">
                <div className="max-w-4xl mx-auto px-6 md:px-0">
                    <Button variant="ghost" size="sm" asChild className="mb-8 -ml-4 text-muted-foreground hover:text-foreground">
                        <Link href="/tutorials">
                            <ArrowLeft size={16} className="mr-2" /> Back to Tutorials
                        </Link>
                    </Button>

                    <div className="flex items-center gap-2 mb-4">
                        <span className="px-2 py-0.5 bg-violet-100 text-violet-800 dark:bg-violet-500/20 dark:text-violet-300 text-xs font-bold uppercase tracking-wider rounded">
                            Tutorial
                        </span>
                    </div>
                        ...
                        {project.linkedBlog && (
                            <Button variant="outline" asChild>
                                <Link href={`/blog/${project.linkedBlog}`}>
                                    <BookOpen size={18} className="mr-2" /> Read the Tutorial
                                </Link>
                            </Button>
                        )}
                    </div>
